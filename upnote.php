...
<link rel="stylesheet" href="upnote.css">
...
<section class="container_data">
    ...
    $quer = "select * from $first$last where title='$uptit'";
    $titNote = mysqli_query($con,$quer);
    $arr = mysqli_fetch_assoc($titNote);


    // $s = "select * from $first$last where title='$title'";
...
?>
    ...
    <div class="form-con1">
        ...
        <form action="deletenote.php" method="post">
            <input type="text" name="title" placeholder="Enter Title To Delete" ?> ">
            <button class="btn" type="submit" placeholder="Delete">Delete</button>                
        </form>
    </div>
    <div class="form_up" >
        <i class="cross uil uil-times"></i>
        <form action="upnote1.php" method="post">
